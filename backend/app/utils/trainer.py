...
class Trainer:
    ...
    def __init__(
        ...
        self.experiment_manager = ExperimentManager(
            experiment_id=int(self.experiment_id.split('_')[1]),
    ...
    def train(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 保存训练配置到文件
            try:
                self.experiment_manager.save_config(self.config)
                self.logger.info(f"训练配置已保存到: {self.experiment_manager.config_file}")
            except Exception as e:
            ...
            for epoch in range(self.current_epoch, self.total_epochs):
                ...
                # 保存指标到文件（持久化）
                try:
                    self.experiment_manager.save_metrics(metrics_entry)
                except Exception as e:
                    self.logger.warning(f"保存指标到文件失败: {e}")

                # 同时保存到内存列表（用于训练结束时保存完整历史）
                self._metrics_history.append(metrics_entry)
            ...
            try:
                ...
                try:
                    ...
                    if training_run:
                        # 只有在状态不是 stopped 时才更新为 completed
                        if training_run.status != "stopped":
                            ...
                            self.logger.info(f"训练已被停止，保持stopped状态: id={training_run_id}")
                finally:
                    db.close()
            except Exception as e:
                self.logger.error(f"更新数据库完成状态失败: {e}")

            # 检查当前状态，只有在不是 stopped 时才更新为 completed
            session = training_logger.get_session(self.experiment_id)
            current_status = session.get("status") if session else None
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, Any]:
        ...
        log_entry = {
            "level": "INFO",
            "message": f"开始 Epoch {epoch + 1}/{self.total_epochs}",
            "source": "trainer",
            "timestamp": datetime.utcnow().isoformat() + "Z"
        }
        training_logger.add_log(
        ...
        self.experiment_manager.append_log(log_entry)
        ...
        if self.task_type == "classification":
            ...
            self.experiment_manager.append_log({
                "level": "INFO",
                "message": log_message,
                "source": "trainer",
                "timestamp": datetime.utcnow().isoformat() + "Z"
    ...
    def _save_checkpoint(self, epoch: int, metrics: Dict[str, Any]):
        ...
        # 使用实验目录管理器获取checkpoint路径
        checkpoint_file = self.experiment_manager.get_checkpoint_path(epoch=epoch)
        best_file = self.experiment_manager.get_checkpoint_path(best=True)

        # 确保目录存在
        self.experiment_manager.checkpoints_dir.mkdir(parents=True, exist_ok=True)
        ...
        training_logger.add_log(
            self.experiment_id,
            "INFO",
            f"Checkpoint已保存: {checkpoint_file.name} (最佳: {is_best})",
            "trainer"
        )
        ...
        self.experiment_manager.append_log({
            "level": "INFO",
            "message": f"Checkpoint已保存: {checkpoint_file.name} (最佳: {is_best})",
            "source": "trainer",
            "timestamp": datetime.utcnow().isoformat() + "Z"
        })
    ...
    def _check_signal(self) -> bool:
        ...
        同时检查内部信号和 training_logger 中的状态。

        Returns:
            True表示应该中断训练
        """
        # 首先检查内部信号（通过 pause()、stop() 方法设置）
        if self.signal == TrainingSignals.PAUSE:
            self.logger.info("训练已暂停（内部信号）")
            self._handle_pause()
            return True

        elif self.signal == TrainingSignals.STOP:
            self.logger.info("训练已停止（内部信号）")
            self._handle_stop()
            return True

        # 检查 training_logger 中的状态（通过 control_training Celery任务更新）
        session = training_logger.get_session(self.experiment_id)
        if session:
            status = session.get("status")

            if status == "paused":
                self.logger.info("检测到暂停状态，等待恢复...")
                self._handle_pause()
                ...
                while waited < max_wait:
                    ...
                    if session and session.get("status") == "stopped":
                        self.logger.info("训练被停止")
                        self._handle_stop()
                        return True

                # 超时后停止
                self.logger.warning("暂停超时，停止训练")
                self._handle_stop()
                return True

            elif status == "stopped":
                self.logger.info("检测到停止状态")
                self._handle_stop()
                return True
