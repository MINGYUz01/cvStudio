...
- [ ] 设置开发环境配置文件（.env、config.py）
- [ ] 创建基本的路由结构（main.py）
- [ ] 配置CORS和中间件

#### 前端任务
- [ ] 初始化React项目（使用Vite）
- [ ] 安装必要的依赖（React Router、Ant Design、Axios等）
- [ ] 创建基本的目录结构
...
- [ ] datasets（数据集表）：id, name, path, format, num_images, classes(json), meta(json), created_at
- [ ] models（模型表）：id, name, graph_json, code_path, template_tag, created_at
- [ ] training_runs（训练任务表）：id, name, model_id, dataset_id, hyperparams(json), status, start_time
- [ ] checkpoints（检查点表）：id, run_id, path, epoch, metric_value, saved_at
